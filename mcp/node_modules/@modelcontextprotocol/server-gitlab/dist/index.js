...
import { GitLabForkSchema, GitLabReferenceSchema, GitLabRepositorySchema, GitLabIssueSchema, GitLabMergeRequestSchema, GitLabContentSchema, GitLabCreateUpdateFileResponseSchema, GitLabSearchResponseSchema, GitLabTreeSchema, GitLabCommitSchema, CreateOrUpdateFileSchema, SearchRepositoriesSchema, CreateRepositorySchema, GetFileContentsSchema, PushFilesSchema, CreateIssueSchema, CreateMergeRequestSchema, ForkRepositorySchema, CreateBranchSchema,bugfixInputSchema,bugfixOutputSchema, } from './schemas.js';
...
async function createOrUpdateFile(projectId, filePath, content, commitMessage = "Auto update", branch, previousPath) {
    const encodedPath = encodeURIComponent(filePath);
    const url = `${GITLAB_API_URL}/projects/${encodeURIComponent(projectId)}/repository/files/${encodedPath}`;

    let method = "POST"; // default to creating file
    let commitId = null;
    ...
    try {
        ...
        if (res.ok) {
            ...
            method = "PUT"; // file exists → we’re updating
        }
    } catch (e) {
    ...
    const body = {
        ...
        ...(commitId ? { last_commit_id: commitId } : {}),
...
server.setRequestHandler(ListToolsRequestSchema, async () => {
    return {
        tools: [
            ...
            {
                name: "push_files",
                description: "Push multiple new files only to a GitLab project in a single commit",
            ...
            {
                name: "fix_bug_in_code",
                description: "Fix a bug in a code snippet given the code snippet, language, and bug description",
                inputSchema: zodToJsonSchema(bugfixInputSchema)
            }
        ]
    };
});
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    try {
        ...
        switch (request.params.name) {
            ...
            case "fix_bug_in_code": {
                const args = bugfixInputSchema.parse(request.params.arguments);
                const fixedCode = await fixBugInCode(args.file_content, args.file_language, args.bug_description);
                return { content: [{ type: "text", text: JSON.stringify(fixedCode, null, 2) }] };
            }
            default:
